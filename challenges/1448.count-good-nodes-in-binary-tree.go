 ...
 * }
 */
func recurse(node *TreeNode, path_max int, good_nodes int) int {
	if node == nil {
		return good_nodes
	}
	...
	return recurse(node.Left, path_max, good_nodes) + recurse(node.Right, path_max, good_nodes)
}

func goodNodes(root *TreeNode) int {
	return recurse(root, root.Val, 0)
}
